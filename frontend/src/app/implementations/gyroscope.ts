...
export const gyroscope = {
    start: async (newUpdateInterval = 100) => {
        console.log(
            `[WEB] Gyroscope start with interval: ${newUpdateInterval}ms`
        );

        if (!isBrowser) {
            throw new Error(
                'Gyroscope operations require a browser environment'
            );
        }
    ...
    getCurrentReading: async () => {
        ...
        if (!isGyroscopeRunning) {
            console.warn(
                'Gyroscope is not running. Start it first to get readings.'
            );
            return { x: 0, y: 0, z: 0 };
        }
