import type { SpeechToTextAPI } from "@/modules/module_types/speechToText";
...
const initializeSpeechRecognition = (): SpeechRecognition | null => {
  ...
  speechRecognition.onresult = (event: SpeechRecognitionEvent) => {
    ...
    console.log("[WEB] Speech recognition interim result:", interimTranscript);
...
export const speechToText: SpeechToTextAPI = {
    ...
    try {
      ...
      if (error instanceof Error) {
        if (error.message.includes("not-allowed")) {
          ...
          throw new Error("Network error occurred during speech recognition");
        }
      }
  ...
  stop: async () => {
    ...
    try {
      ...
      return (
        finalTranscript.trim() || "Error occurred during speech recognition"
      );
    }
  },
};
