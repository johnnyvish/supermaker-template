import type { LocationAPI } from "@/modules/module_types/location";
...
export const location: LocationAPI = {
  ...
  watchPosition: async (
    ...
    const nativeWatchId = navigator.geolocation.watchPosition(
      ...
      (error) => {
        ...
        sendEventToWebView("locationError", { watchId, error: error.message });
      },
      options
    );
  ...
  getLastKnownPosition: async (maxAge?: number, requiredAccuracy?: number) => {
    console.log(
