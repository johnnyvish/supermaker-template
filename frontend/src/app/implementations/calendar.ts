import type { CalendarAPI } from "@/modules/module_types/calendar";
...
export const calendar: CalendarAPI = {
  ...
  createEvent: async (
    ...
    if (!isBrowser) {
      throw new Error("Calendar operations require a browser environment");
    }
  ...
  getEvents: async (calendarIds: string[], startDate: Date, endDate: Date) => {
    console.log(`[WEB] Calendar getEvents: ${calendarIds.length} calendars`);
