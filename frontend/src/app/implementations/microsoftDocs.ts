...
} from "@/modules/module_types/microsoftDocs";
import {
  Document,
  Packer,
  Paragraph,
          ...
          children: tableData.headers.map(
            (header) =>
              new TableCell({
                children: [
                  new Paragraph({
                    children: [new TextRun({ text: header, ...headerStyle })],
                    alignment:
    ...
    rows.push(
      new TableRow({
        children: row.map((cell) => {
          const content =
            typeof cell === "string"
              ? [new Paragraph({ text: cell, spacing: { after: 60 } })]
...
// Main implementation
export const microsoftDocs: MicrosoftDocsAPI = {
  generateDocument: async (options: SimpleDocumentOptions): Promise<Buffer> => {
              ...
              size:
                options.orientation === "landscape"
                  ? {
                      ...
                      width: convertInchesToTwip(pageSize.height),
                      height: convertInchesToTwip(pageSize.width),
                    }
                  : {
                      width: convertInchesToTwip(pageSize.width),
                      height: convertInchesToTwip(pageSize.height),
                    },
            },
          }
        : undefined;
          ...
          default: new Header({
            children:
              typeof options.header === "string"
                ...
                : [createParagraph(options.header as ParagraphContent)],
          }),
        }
      : undefined;
    ...
    if (options.footer || options.pageNumbers) {
      ...
      if (options.footer) {
        if (typeof options.footer === "string") {
          footerChildren.push(new Paragraph({ text: options.footer }));
        } else {
          footerChildren.push(
  ...
  readDocument: async (
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          console.error("Mammoth final conversion error:", finalError);
          throw new Error(
    ...
    htmlLines.forEach((line) => {
      ...
      if (line.includes("<h1")) {
        ...
        // Detect text styling
        const style: { bold?: boolean; italic?: boolean; underline?: boolean } =
          {};
        if (line.includes("<strong>") || line.includes("<b>"))
          style.bold = true;
        if (line.includes("<em>") || line.includes("<i>")) style.italic = true;
    ...
    const tableMatches = result.value.match(/<table[^>]*>.*?<\/table>/g) || [];
    tableMatches.forEach((tableHtml) => {
      ...
      rowMatches.forEach((rowHtml) => {
        const cells: string[] = [];
        const cellMatches = rowHtml.match(/<t[dh][^>]*>.*?<\/t[dh]>/g) || [];
  ...
  modifyDocument: async (
        ...
        case "replaceParagraph":
          if (mod.target?.index !== undefined && mod.content) {
            ...
            if (targetPara) {
              ...
              const idx = structure.paragraphs.indexOf(targetPara);
        ...
        case "replaceTable":
          if (mod.target?.index !== undefined && mod.content) {
            ...
            if (
              ...
              processedIndices.add(mod.target.index);
              targetTable.headers = mod.content.headers || targetTable.headers;
              targetTable.rows = mod.content.rows || targetTable.rows;
            }
          }
          break;
    ...
    const allElements: (
      | (DocumentStructure["paragraphs"][0] & { elementType: "paragraph" })
      ...
      ...structure.tables.map((t) => ({ ...t, elementType: "table" as const })),
    ...
    const metadataMod = modifications.find((m) => m.type === "updateMetadata");
