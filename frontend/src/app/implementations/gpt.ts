import { MEDIUM_GPT_MODEL, DEFAULT_GPT_MODEL } from "@/config";
import { callProxy } from "@/utils";
import type { GPTAPI } from "@/modules/module_types/gpt";
import type {
  ProxyResponse,
 ...
 */
const buildToolsSystemPrompt = (toolkits: string[], userSystemPrompt?: string) => {
...
export const ask = async (
  ...
  try {
    ...
    const content = proxyResponse.data?.choices?.[0]?.message?.content || "";
    ...
    if (toolkits && toolkits.length > 0) {
      for (const toolkit of toolkits) {
        try {
          ...
          if (toolDefsResponse.data && toolDefsResponse.data.length > 0) {
          ...
          if (authRequired) {
            ...
            if (authResponse?.status === "completed") {
              try {
                ...
                const retryResult = (
                  retryProxyResult as ProxyResponse<{ output: unknown }>
                ).data;
              ...
              if (authResponse?.url && typeof window !== "undefined") {
